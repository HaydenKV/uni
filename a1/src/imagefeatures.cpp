...
{
    return detectAndDrawShiAndTomasi(img, maxNumFeatures, nullptr);
}
                                  ...
                                  std::vector<cv::Point2f>* outPoints)
{
    ...
    cv::Mat gray;
    if (img.channels() == 1) gray = img; else cv::cvtColor(img, gray, cv::COLOR_BGR2GRAY);
    ...
    for (int y = 0; y < minEigenResponse.rows; ++y) {
        const float* row = minEigenResponse.ptr<float>(y);
        for (int x = 0; x < minEigenResponse.cols; ++x) {
            float s = row[x];
            if (s > threshold) corners.push_back({cv::Point(x,y), s});
        }
    }
    ...
    const int count = std::min(maxNumFeatures, static_cast<int>(corners.size()));

    // draw all (orange)
    for (const auto& c : corners) {
        cv::circle(imgout, c.pt, 3, cv::Scalar(0,165,255), -1);
    }
    // top-N highlight + labels
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (outPoints) outPoints->push_back(cv::Point2f((float)c.pt.x, (float)c.pt.y));
    }
...
{
    return detectAndDrawFAST(img, maxNumFeatures, nullptr);
}
                          ...
                          std::vector<cv::Point2f>* outPoints)
{
    ...
    const int count = std::min(maxNumFeatures, static_cast<int>(keypoints.size()));
    ...
    for (int i = 0; i < count; ++i) {
        ...
        cv::putText(imgout, std::to_string(i), kp.pt + cv::Point2f(5,5),
                    cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0,255,0), 2);
        if (outPoints) outPoints->push_back(kp.pt);
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// One-call wrapper to choose the detector (handy for your main loop)
cv::Mat detectAndDrawFeatures(const cv::Mat& img,
                              FeatureDetector which,
                              int maxNumFeatures,
                              std::vector<cv::Point2f>* outPoints)
{
    switch (which) {
        case FeatureDetector::ShiTomasi:
            return detectAndDrawShiAndTomasi(img, maxNumFeatures, outPoints);
        case FeatureDetector::FAST:
            return detectAndDrawFAST(img, maxNumFeatures, outPoints);
        default:
            return detectAndDrawShiAndTomasi(img, maxNumFeatures, outPoints);
    }
}
