...
bool individualCompatibility(const Eigen::Vector2d & y, const GaussianInfo<double> & marginal, const double & nSigma)
{
    ...
    Eigen::VectorXd ydyn = y;
    return marginal.isWithinConfidenceRegion(ydyn, nSigma);
}
...
bool jointCompatibility(const std::vector<int> & idx, const double & sU, const Eigen::Matrix<double, 2, Eigen::Dynamic> & Y, const GaussianInfo<double> & density, const double & nSigma, double & surprisal)
{
    ...
    if (nA > 0)
    {
        ...
        // 1) Stack the associated measurements: yA ∈ R^{2 nA}
        Eigen::VectorXd yA(2 * nA);
        for (int a = 0; a < nA; ++a)
        {
            yA.segment<2>(2*a) = Y.col(idxi[a]);   // Y is [2 x m]
        }

        // 2) Marginal over the associated predicted components [2*k, 2*k+1]
        GaussianInfo<double> densA = density.marginal(idxyj);

        // 3) Joint gate using the class' chi-square test (k = 2*nA)
        const bool compatible = densA.isWithinConfidenceRegion(yA, nSigma);

        // 4) Total surprisal = unassociated uniform + associated Gaussian NLL
        const double sA = -densA.log(yA);
        surprisal = nU * sU + sA;
