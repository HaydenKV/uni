...
function tests = test_GaussianInfoConditional
...
function runConditionalCase(testCase, p, idxA, idxB, xB)
    ...
    pB = GaussianInfo.fromSqrtMoment(xB, triu(magic(length(xB))));
...
verifyEqual(testCase, muc_actual, muc_expected, 'Expected conditional mean to match expected result', 'AbsTol', 1e-10);

Pc_actual = pc.cov(); % Sc_actual.'*Sc_actual
Pc_expected = P(idxA, idxA) - P(idxA, idxB)*(P(idxB, idxB)\P(idxB, idxA));
verifyEqual(testCase, Pc_actual, Pc_expected, 'Expected conditional cov to match expected result', 'AbsTol', 1e-10);
end
...
verifyEqual(testCase, Pc_actual, Pc_expected, 'Expected conditional cov to match expected result', 'AbsTol', 1e-9);
end
