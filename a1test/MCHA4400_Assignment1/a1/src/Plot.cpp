...
void Plot::render()
{
    ...
    auto applyPreset = [&](int preset){
        ...
        if (preset == 1) {
            ...
            cam->SetFocalPoint(0,0,2);
            double sc = 25.0;
            cam->SetPosition(-0.8*sc, -0.8*sc, -0.5*sc);
            cam->SetViewUp(-1,0,0);
        } else {
            // Scenario 3 (points)
            cam->Azimuth(0);
    ...
    for (std::size_t i = 0; i < pSystem->numberLandmarks(); ++i)
    {
        ...
        } else if (const auto* ptMeas = dynamic_cast<const MeasurementPointBundle*>(pMeasurement.get())) {
            const auto& idxF = ptMeas->idxFeatures();
            if (i < idxF.size()) isDetected = (idxF[i] >= 0);   // turn BLUE when matched
            isIdentifiable = true;
        }else {
